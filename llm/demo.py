# demo.py - Instruction-Aligned LLM + RAG Test Step Generator
import json
import os
import sys
import re
...
class StepExtractor:
    ...
    def extract_steps_from_output(output_text):
        ...
        if not steps:
            sentences = [s.strip() for s in output_text.split('.') if len(s.strip()) > 5]
...
# ----------------------------------
# INSTRUCTION-BASED VALIDATOR (NEW)
# ----------------------------------
class InstructionValidator:
    """
    Validates steps by checking whether they satisfy
    the requirements extracted from the instruction.
    """

    def extract_requirements(self, instruction):
        ...
        # Input count
        if "2 numbers" in instruction:
            req["inputs"] = 2
        elif "3 numbers" in instruction:
            req["inputs"] = 3
        ...
        # UI requirement
        if "app" in instruction or "form" in instruction:
            req["needs_ui"] = True

        # Game
        if "2048" in instruction or "game" in instruction:
            req["game"] = True

        return req

    def validate(self, instruction, steps):
        requirements = self.extract_requirements(instruction)
        issues = []
        action_text = " ".join(s["action"].lower() for s in steps)

        # Validate inputs
        if requirements["inputs"] > 0:
            input_mentions = action_text.count("input")
            if input_mentions < requirements["inputs"]:
                issues.append(
                    f"Expected {requirements['inputs']} input fields, found {input_mentions}"
                )

        # Validate operations
        for op in requirements["operations"]:
            if op not in action_text:
                issues.append(f"Missing required operation: {op}")

        # Validate UI
        if requirements["needs_ui"]:
            if not any(w in action_text for w in ["button", "input", "field"]):
                issues.append("UI elements not clearly defined")

        # Game validation
        if requirements["game"]:
            if not any(w in action_text for w in ["swipe", "arrow", "tile", "move"]):
                issues.append("Game-related actions missing")

        alignment_score = self._alignment_score(instruction, steps)

        is_valid = len(issues) == 0 and alignment_score >= 0.3
        confidence = round(min(1.0, 0.5 + alignment_score), 2)
    ...
    def _alignment_score(self, instruction, steps):
        instr_words = set(re.findall(r"\w+", instruction.lower()))
        step_words = set(
            word
            for s in steps
            for word in re.findall(r"\w+", s["action"].lower())
        )
        overlap = instr_words & step_words
        return len(overlap) / max(len(instr_words), 1)
...
class SimpleRAGSystem:
    def __init__(self, dataset_path):
        ...
        self.instructions = [d["instruction"] for d in self.dataset]
    ...
    def retrieve(self, query):
        ...
        idx = np.argmax(sims)
        return self.dataset[idx] if sims[idx] > 0.25 else None
...
class SimpleAssistant:
    def __init__(self):
        ...
        self.rag_general = SimpleRAGSystem(os.path.join(base, "llm_dataset.jsonl"))
        self.rag_2048 = SimpleRAGSystem(os.path.join(base, "rag_2048.jsonl"))
        self.validator = InstructionValidator()
        self.extractor = StepExtractor()

        # Load fine-tuned model (optional)
        self.tokenizer = None
        self.model = None
        model_dir = os.path.join(base, "fine_tuned_js_model")
    ...
    def get_category(self, text):
        t = text.lower()
        if "2048" in t or "game" in t:
            return "game"
        return "software"

    def generate_steps(self, instruction, category):
        if category == "game":
            match = self.rag_2048.retrieve(instruction)
            if match:
                return self.extractor.extract_steps_from_output(match["output"])
        ...
        if self.model:
            ...
            steps = self.extractor.extract_steps_from_output(text)
            if steps:
                return steps

        # RAG fallback
        match = self.rag_general.retrieve(instruction)
        if match:
            return self.extractor.extract_steps_from_output(match["output"])

        return [{"step": 1, "action": "Analyze requirement"}]

    def process(self, instruction):
        category = self.get_category(instruction)
        steps = self.generate_steps(instruction, category)
        validation = self.validator.validate(instruction, steps)
    ...
    def interactive(self):
        while True:
            q = input("\nEnter instruction (or quit): ")
            if q.lower() in ["quit", "exit"]:
                break
            result = self.process(q)
